defmodule NewPassport do
  def get_new_passport(now, birthday, form) do
    with {:ok, unix_time} <- enter_building(now),
         {:ok, counter_fn} <- find_counter_information(now),
         counter <- counter_fn.(birthday),
         {:ok, checksum} <-
           stamp_form(unix_time, counter, form) do
      {:ok, get_new_passport_number(unix_time, counter, checksum)}
    else
      {:error, message} -> {:error, message}
      {:coffee_break, _} -> {:retry, NaiveDateTime.add(now, 15 * 60, :seconds)}
    end
  end
